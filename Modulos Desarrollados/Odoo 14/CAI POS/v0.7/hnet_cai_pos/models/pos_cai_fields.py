...
class PosConfig(models.Model):
    ...
    # Secuencia POS_CAI
    pos_order_secuencia_prefijo = fields.Char('Prefijo de la secuencia de la orden del POS')
    # Prefijo Ej, 000-001-01-00
    pos_order_secuencia_id = fields.Many2one('ir.sequence', 'Secuencia de la orden del POS')

    @api.model
    def _update_pos_order_secuencia_id(self, values):
        """
        Actualizar la secuencia de la orden utilizando el prefijo de la secuencia
            y rango autorizado
        """
        prefix = values.get('pos_order_secuencia_prefijo')
        if not prefix:
            return values

        seq_id = values.get('pos_order_secuencia_id')
        if not seq_id:
            ...
            values.update({'pos_order_secuencia_id':seq_id})
        else:
    ...
    def create(self, values):
        ...
        """
        self._update_pos_order_secuencia_id(values)
    ...
    def write(self, vals):
        ...
        for conf in self:
            values = vals.copy()
            if conf.pos_order_secuencia_id:
                values.update({'pos_order_secuencia_id': conf.pos_order_secuencia_id.id})
            self._update_pos_order_secuencia_id(values)
