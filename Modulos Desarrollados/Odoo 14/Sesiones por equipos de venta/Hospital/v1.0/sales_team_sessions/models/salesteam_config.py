...
class SalesTeamConfig(models.Model):
    ...
    def _default_sale_journal(self):
        return self.env['account.journal'].search([('type', '=', 'sale'), ('company_id', '=', self.env.company.id), ('code', '=', 'FV')], limit=1)
        ...
        if len(cash_method) == 1:
            ...
            for method in cash_method:
                ...
                for idx, name in enumerate(split_name):
                    team_name = self.crm_team_id
                    if name == (self.crm_team_id.name).upper():
            ...
            if len(cash_method) != 1:               
                cash_method = self.env['account.journal'].search([('company_id', '=', self.env.company.id), ('type', '=', 'cash'), ('default_account_id', '>=', 1)], limit=1)
            
                for method in cash_method:
                    payment_methods.append(method)
                
            else:
                payment_methods.append(cash_method)
