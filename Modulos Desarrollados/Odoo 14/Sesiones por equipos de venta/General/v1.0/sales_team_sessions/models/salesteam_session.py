...
# cambiar receivable_account_id por default_account_id
class SalesTeamSession(models.Model):
    _name = 'salesteam.session'
    _order = 'id desc'
    _description = 'Sales Team Session'

    SALES_TEAM_SESSION_STATE = [
        ('opening_control', 'Control De Apertura'),  # method action_salesteam_session_open
        ('opened', 'En Progreso'),               # method action_salesteam_session_closing_control
        ('closing_control', 'Control de Cierre'),  # method action_salesteam_session_close
        ('closed', 'Cerrado & Publicado'),
    ]

    #### ------------------------------------------------- Campos del Modelo ------------------------------------------------ ####
    #### ---------------------------------------------------------------------------------------------------------------------####
    company_id = fields.Many2one('res.company', related='config_id.company_id', string="Compañía", readonly=True)

    config_id = fields.Many2one(
        'salesteam.config', string='Configuración de la sesión',
        help="The sale session for your sales team.",
        required=True,
        index=True)
    name = fields.Char(string='ID Sesión', required=True, readonly=True, default='/')
    ...
    state = fields.Selection(
        SALES_TEAM_SESSION_STATE, string='Estado',
        required=True, readonly=True,
        index=True, copy=False, default='opening_control')
    crm_team_id = fields.Many2one('crm.team', related='config_id.crm_team_id', string="Equipo de Ventas", readonly=True)
    # logged_employee_id = fields.Many2one('hr.employee', string='Current Session Employee', required=True)

    sequence_number = fields.Integer(string='Nombre de la secuencia de orden', help='A sequence number that is incremented with each order', default=1)
    login_number = fields.Integer(string='Numero de ingresos a la sesión', help='A sequence number that is incremented each time a user resumes the sales session', default=1)
    ...
    cash_register_id = fields.Many2one('account.bank.statement', compute='_compute_cash_all', string='Registro de efectivo', store=True)

    cash_register_balance_end_real = fields.Monetary(
        related='cash_register_id.balance_end_real',
        string="Balance de cierre",
    ...
    cash_register_total_entry_encoding = fields.Monetary(
        ...
        string='Total de efectivo transaccionado',
        readonly=True,
        help="Total of all paid sales orders")
    cash_register_balance_end = fields.Monetary(
        compute='_compute_cash_balance',
        string="Balance de cierre teorico",
        help="Sum of opening balance and transactions.",
        readonly=True)
    cash_register_difference = fields.Monetary(
        compute='_compute_cash_balance',
        string='Diferencia Antes de Cerrar',
        help="Difference between the theoretical closing balance and the real closing balance.",
        readonly=True)
    cash_real_difference = fields.Monetary(string='Diferencia', readonly=True, default=0.00)
    cash_real_transaction = fields.Monetary(string='Transacción', readonly=True)
    cash_real_expected = fields.Monetary(string="Esperado", readonly=True)

    order_ids = fields.One2many('sale.order', 'salesteam_session_id',  string='Ordenes')
    order_count = fields.Integer(compute='_compute_order_count')
    statement_ids = fields.One2many('account.bank.statement', 'salesteam_session_id', string='Estados del efectivo', readonly=True)
    failed_pickings = fields.Boolean(compute='_compute_picking_count')
    picking_count = fields.Integer(compute='_compute_picking_count')
    picking_ids = fields.One2many('stock.picking', 'salesteam_session_id')
    rescue = fields.Boolean(string='Recuperar sesión',
        help="Auto-generated session for orphan orders, ignored in constraints",
        readonly=True,
        copy=False)
    move_id = fields.Many2one('account.move', string='Diario de entradas')
    payment_method_ids = fields.Many2many('account.journal', related='config_id.payment_method_ids', string='Métodos de Pago')
    total_payments_amount = fields.Float(compute='_compute_total_payments_amount', string='Monto total de pagos')
    is_in_company_currency = fields.Boolean('Se está usando la moneda de la compañía', compute='_compute_is_in_company_currency')
    #update_stock_at_closing = fields.Boolean('Stock should be updated at closing')

    _sql_constraints = [('uniq_name', 'unique(name)', "¡El nombre de esta sesión del equipo de ventas debe ser único!")]

    current_user = fields.Boolean(string="Usuario responsable loggeado", default=False)

    #### ------------------------------------------------- Campos Calculados ------------------------------------------------ ####
    #### ---------------------------------------------------------------------------------------------------------------------####
    @api.depends('login_number')
    def _get_current_user(self, logged_user):
        for session in self:
    ...
    def _compute_is_in_company_currency(self): ### REVISADA
        for session in self:
    ...
    def _compute_cash_balance(self): ### REVISADA
        for session in self:
            cash_payment_method = session.payment_method_ids.filtered(lambda pm: True if (pm.type == 'cash') else False)[:1]
            if cash_payment_method:
                total_cash_payment = 0.0
                result = self.env['salesteam.payment'].read_group([('salesteam_session_id', '=', session.id), ('payment_method_id', '=', cash_payment_method.id)], ['amount'], ['salesteam_session_id'])
                if result:
    ...
    def _compute_total_payments_amount(self): ### REVISADA
        result = self.env['salesteam.payment'].read_group([('salesteam_session_id', 'in', self.ids)], ['amount'], ['salesteam_session_id'])
        session_amount_map = dict((data['salesteam_session_id'][0], data['amount']) for data in result)
        for session in self:
            session.total_payments_amount = session_amount_map.get(session.id) or 0

    def _compute_order_count(self): ### REVISADA
        orders_data = self.env['sale.order'].read_group([('salesteam_session_id', 'in', self.ids)], ['salesteam_session_id'], ['salesteam_session_id'])
        sessions_data = {order_data['salesteam_session_id'][0]: order_data['salesteam_session_id_count'] for order_data in orders_data}
        for session in self:
            session.order_count = sessions_data.get(session.id, 0)

    @api.depends('picking_ids', 'picking_ids.state')
    def _compute_picking_count(self): ### REVISADA
        for session in self:
            session.picking_count = self.env['stock.picking'].search_count([('salesteam_session_id', '=', session.id)])
            session.failed_pickings = bool(self.env['stock.picking'].search([('salesteam_session_id', '=', session.id), ('state', '!=', 'done')], limit=1))

    def action_stock_picking(self): ### REVISADA
        self.ensure_one()
    ...
    def _compute_cash_all(self): ### REVISADA
        # Only one cash register is supported by salesteam:session.
        for session in self:
            session.cash_journal_id = session.cash_register_id = session.cash_control = False
            cash_payment_methods = session.payment_method_ids.filtered(lambda pm: True if (pm.type == 'cash') else False)
            if not cash_payment_methods:
                continue
            for statement in session.statement_ids:
                if cash_payment_methods[0].type == 'cash':
                    if statement.journal_id == cash_payment_methods:
    ...
    def _check_salesteam_config(self): ### REVISADA
        if self.search_count([
                ('state', '!=', 'closed'),
                ('config_id', '=', self.config_id.id),
                ('rescue', '=', False)
            ]) > 1:
            raise ValidationError(_("Ya está abierta otra sesión para este equipo de ventas."))

    @api.constrains('start_at')
    def _check_start_date(self): ### REVISADA
        for record in self:
            ...
            if (company.period_lock_date and start_date <= company.period_lock_date) or (company.fiscalyear_lock_date and start_date <= company.fiscalyear_lock_date):
                raise ValidationError(_("No puede crear una sesión antes de la fecha de bloqueo contable."))

    def _check_bank_statement_state(self): ### REVISADA
        for session in self:
            closed_statement_ids = session.statement_ids.filtered(lambda x: x.state != "open")
            if closed_statement_ids:
                raise UserError(_("Algunas cajas registradoras ya están publicadas. Restablezcalos a nuevos para cerrar la sesión.\n"
                                  "Regristros de efectivo: %r", list(statement.name for statement in closed_statement_ids)))

    def _check_salesteam_session_balance(self): ### REVISADA
        for session in self:
            for statement in session.statement_ids:
                if (statement != session.cash_register_id) and (statement.balance_end != statement.balance_end_real):
                    statement.write({'balance_end_real': statement.balance_end})
    
    def _validate_session(self): ### REVISADA
        self.ensure_one()
        sudo = self.user_has_groups('sales_team_sessions.group_employee_session')
        if self.order_ids or self.statement_ids.line_ids:
            ...
            if self.move_id.line_ids:
                ...
                self.env['sale.order'].search([('salesteam_session_id', '=', self.id), ('state', '=', 'draft')]).write({'state': 'sale'})
            else:
                self.move_id.unlink()
        else:
    ...
    def _check_if_no_draft_orders(self):
        ...
        if draft_orders:
            raise UserError(_(
                    'Todavía hay órdenes en estado de borrador en la sesión. '
                    'Paga o cancela los siguientes pedidos para validar la sesión:\n%s'
    ...
    def create(self, values):
        ...
        if not config_id:
            raise UserError(_("Debe asignar una configuración de equipo de ventas a su sesión."))

        # journal_id is not required on the salesteam_config because it does not
        # exists at the installation. If nothing is configured at the
        # installation we do the minimal configuration. Impossible to do in
        # the .xml files as the CoA is not yet installed.
        salesteam_config = self.env['salesteam.config'].browse(config_id)
        ctx = dict(self.env.context, company_id=salesteam_config.company_id.id)

        salesteam_name = self.env['ir.sequence'].with_context(ctx).next_by_code('salesteam.session')
        if values.get('name'):
            salesteam_name += ' ' + values['name']

        cash_payment_methods = salesteam_config.payment_method_ids.filtered(lambda pm: True if (pm.type == 'cash') else False)
        statement_ids = self.env['account.bank.statement']
        if self.user_has_groups('sales_team_sessions.group_employee_session'):
            statement_ids = statement_ids.sudo()
        for cash_journal in cash_payment_methods.mapped('type' == 'cash'):
            ctx['journal_id'] = cash_journal.id if salesteam_config.cash_control and cash_journal.type == 'cash' else False
            st_values = {
        ...
        #update_stock_at_closing = salesteam_config.company_id.point_of_sale_update_stock_quantities == "closing"
        ...
        if self.user_has_groups('sales_team_sessions.group_employee_session'):
            res = super(SalesTeamSession, self.with_context(ctx).sudo()).create(values)
        else:
            res = super(SalesTeamSession, self.with_context(ctx)).create(values)
        res.action_salesteam_session_open()
    ...
    def unlink(self):
        ...
        return super(SalesTeamSession, self).unlink()

    #### ----------------------------------------- Acciones de la sesión de ventas ------------------------------------------ ####
    #### ---------------------------------------------------------------------------------------------------------------------####
    def login(self): ### REVISADA
        self.ensure_one()
    ...
    def action_salesteam_session_open(self): ### REVISADA
        ...
        for session in self.filtered(lambda session: session.state in ('new_session', 'opening_control')):
            ...
            if session.config_id.cash_control and not session.rescue:
                last_sessions = self.env['salesteam.session'].search([('config_id', '=', self.config_id.id)]).ids
                # last session includes the new one already.
                self.cash_register_id.balance_start = self.env['salesteam.session'].browse(last_sessions[1]).cash_register_id.balance_end_real if len(last_sessions) > 1 else 0
    ...
    def action_salesteam_session_closing_control(self): ### REVISADA
        if not self.current_user:
            raise UserError(_('Solo el responsable de la sesión de ventas puede realizar esta acción'))

        self._check_salesteam_session_balance()
        for session in self:
            if any(order.state == 'draft' for order in session.order_ids):
                raise UserError(_("No puede cerrar la sesión del equipo de ventas cuando los pedidos aún están en borrador"))
            ### No se puede cerrar sesión de ventas si se tienen ordenes sin facturar
            if any(order.invoice_status != 'invoiced' for order in session.order_ids):
                raise UserError(_("No puede cerrar la sesión del equipo de ventas cuando los pedidos aún deben facturarse"))
            if session.state == 'closed':
                raise UserError(_('Esta sesión ya está cerrada.'))
            session.write({'state': 'closing_control', 'stop_at': fields.Datetime.now()})
            if not session.config_id.cash_control:
                session.action_salesteam_session_close()

    def action_salesteam_session_validate(self): ### REVISADA
        if not self.current_user:
            raise UserError(_('Solo el responsable de la sesión de ventas puede realizar esta acción'))
        self._check_salesteam_session_balance()
        return self.action_salesteam_session_close()

    def action_salesteam_session_close(self): ### REVISADA
        ...
        if self.cash_control and abs(self.cash_register_difference) > self.config_id.amount_authorized_diff:
            # Only salesman close statements with cash_register_difference greater than amount_authorized_diff.
            if not self.user_has_groups("sales_team_sessions.group_admin_session"):
                raise UserError(_(
                    "Su saldo final es demasiado diferente del cierre de caja teórico (%.2f), "
                    "el máximo permitido es: %.2f."
    ...
    #### ---------------------------------------------------------------------------------------------------------------------####
    def _create_account_move(self): ### REVISADA 
    ...
    def _accumulate_amounts(self, data): ### REVISADA
        # Accumulate the amounts for each accounting lines group
        # Each dict maps `key` -> `amounts`, where `key` is the group key.
        # E.g. `combine_receivables` is derived from salesteam.payment records
        # in the self.order_ids with group key of the `payment_method_id`
        # field of the salesteam.payment record.
        ...
        for order in self.order_ids:
            # Combine sales receivable lines
            # Separate cash payments for cash reconciliation later.
            for payment in order.payment_ids:
                amount, date = payment.amount, payment.payment_date
                """
                if payment.payment_method_id.split_transactions:
                    if payment.payment_method_id.filtered(lambda pm: True if (pm.type == 'cash') else False):
            ...
            if order.invoice_status	== 'invoiced':
                ...
                raise UserError(_("No puede cerrar la sesión del equipo de ventas cuando los pedidos aún deben facturarse"))
                """ No se debe poder cerrar la sesión si hay ordenes sin facturar ni pagar 
        ...
        if self.company_id.anglo_saxon_accounting:
            global_session_pickings = self.picking_ids.filtered(lambda p: not p.sale_order_id)
    ...
    def _create_non_reconciliable_move_lines(self, data): ### REVISADA
        ...
        if len(tax_names_no_account) > 0:
            error_message = _(
                'No se pudo cerrar y validar la sesión.\n'
    ...
    def _create_cash_statement_lines_and_cash_move_lines(self, data): ### REVISADA
        ...
        for payment, amounts in split_receivables_cash.items():
            statement = statements_by_journal_id[payment.payment_method_id.id]
            split_cash_statement_line_vals[statement].append(self._get_statement_line_vals(statement, payment.payment_method_id.default_account_id, amounts['amount'], date=payment.payment_date, partner=payment.sale_order_id.partner_id))
        ...
        for payment_method, amounts in combine_receivables_cash.items():
            if not float_is_zero(amounts['amount'] , precision_rounding=self.currency_id.rounding):
                statement = statements_by_journal_id[payment_method.id]
                combine_cash_statement_line_vals[statement].append(self._get_statement_line_vals(statement, payment_method.default_account_id, amounts['amount']))
    ...
    def _create_balancing_line(self, data): ### REVISADA
        imbalance_amount = 0
    ...
    def _prepare_balancing_line_vals(self, imbalance_amount, move): ### REVISADA
        account = self._get_balancing_account()
        partial_vals = {
            'name': _('Cerrar la sesión del equipo de ventas'),
    ...
    def _get_balancing_account(self): ### REVISADA 
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        pickings = self.picking_ids.filtered(lambda p: not p.sale_order_id)
        pickings |= self.order_ids.filtered(lambda o: True if o.invoice_status == 'invoiced' else False).mapped('picking_ids')
    ...
    def _prepare_line(self, order_line):
        ...
        def get_income_account(order_line):
            ...
            if not income_account:
                raise UserError(_('Defina la cuenta de ingresos para este producto:"%s" (id:%d).')
    ...
    def _get_split_receivable_vals(self, payment, amount, amount_converted):
        partial_vals = {
            'account_id': payment.payment_method_id.default_account_id.id,
            'move_id': self.move_id.id,
    ...
    def _get_combine_receivable_vals(self, payment_method, amount, amount_converted):
        partial_vals = {
            'account_id': payment_method.default_account_id.id,
            'move_id': self.move_id.id,
    ...
    def _credit_amounts(self, partial_move_line_vals, amount, amount_converted, force_company_currency=False):
        ...
        #NOTE Amounts in Sales Team config are in the currency of journal_id in the session.config_id.
        #This means that amount fields in any Sales Team config record are actually equivalent to amount_currency
        ...
        #:param amount float:
        #    amount derived from salesteam.payment, sale.order, or sale.order.line records
    ...
    def action_show_payments_list(self): ### REVISADA
        # Antes de poder vender debe estar la sesión de ventas abierta 
        current_session_state = self.config_id.current_session_id.state
        if current_session_state == 'opening_control':
            raise ValidationError(_('Debe inciar la sesión de venta antes de empezar a vender.'))
        return {
            'name': _('Pagos'),
            'type': 'ir.actions.act_window',
            'res_model': 'salesteam.payment',
            'view_mode': 'tree,form',
            'domain': [('salesteam_session_id', '=', self.id)],
            'context': {'search_default_group_by_payment_method': 1}
        }

    def open_cashbox_salesteam(self):
        if not self.current_user:
            raise UserError(_('Solo el responsable de la sesión de ventas puede realizar esta acción'))

        self.ensure_one()
        action = self.cash_register_id.open_cashbox_id()
        action['view_id'] = self.env.ref('sales_team_sessions.view_account_bnk_stmt_cashbox_footer').id
        action['context']['salesteam_session_id'] = self.id
        action['context']['default_salesteam_id'] = self.config_id.id
        ### Cambiar el estado de de open_control a opened
        self.config_id.current_session_id.rescue = True
        self.action_salesteam_session_open()
        return action

    def set_cashbox_salesteam(self, cashbox_value, notes):
        self.state = 'opened'
    ...
    def action_view_order(self):
        ...
        current_session_state = self.config_id.current_session_id.state
        if current_session_state == 'opening_control':
            raise ValidationError(_('Debe inciar la sesión de venta antes de poder ver los pagos realizados.'))
        else:
            # Direccionar a la vista Lista de las ordenes de la sesión de ventas
            return {
                ...
                'type': 'ir.actions.act_window',
                'domain': [('salesteam_session_id', 'in', self.ids)],
            }
    ...
    def _warning_balance_closing(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'closing.balance.confirm.wizard.salesteam',
            'views': [(False, 'form')],
            'type': 'ir.actions.act_window',
            'context': context,
            'target': 'new'
...
class ClosingBalanceConfirm(models.TransientModel):
    _name = 'closing.balance.confirm.wizard.salesteam'
    ...
    def confirm_closing_balance(self):
        current_session =  self.env['salesteam.session'].browse(self._context['session_id'])
