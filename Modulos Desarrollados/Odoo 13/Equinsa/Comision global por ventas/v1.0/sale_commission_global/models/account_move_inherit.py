...
class AccountMove(models.Model):
    ...
    commission_total_global = fields.Float(
        string="Comisión Global", compute="_compute_commission_total_global", store=True,
    )

    commission_total_salesman = fields.Float(
        string="Comisión Global Salesman", default=0.00, store=True,
    )

    commission_total_manager = fields.Float(
        string="Comisión Global Manager", default=0.00, store=True,
    )
    ...
    def _compute_commission_total_global(self):
        for record in self:
            ...
            for line in record.invoice_line_ids:
                expenses += line.product_id.standard_price
                order_line_ids.append(line.id)

            # Calcular la comisión global del agente 
            if expenses > 0 and record.amount_untaxed > 0:
                margin_percent = (expenses/record.amount_untaxed) * 100
