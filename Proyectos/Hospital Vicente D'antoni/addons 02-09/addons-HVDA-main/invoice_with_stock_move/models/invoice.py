    ...
    def action_post(self):
        ...
        for inv in self:
            if inv.create_stock_moves:
                picking_type_id = inv.picking_type_id
                acs_location_id = inv.acs_location_id
                acs_location_dest_id = inv.acs_location_dest_id
                if (not picking_type_id) or (not acs_location_id) or (not acs_location_dest_id):
                    inv.onchange_warehouse()

                if picking_type_id and acs_location_id and acs_location_dest_id:
                    inv.acs_create_picking(picking_type_id, acs_location_id, acs_location_dest_id)
