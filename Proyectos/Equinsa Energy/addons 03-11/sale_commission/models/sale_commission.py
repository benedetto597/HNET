...
class SaleCommission(models.Model):
    ...
    section_type = fields.Selection(
        selection=[("amount", "Por monto"), ("percentage", "Por porcentaje")],
    ...
    def calculate_section(self, base, margen=0.0):
        self.ensure_one()

        if self.section_type == "percentage":
            try:
                utility_percentage = (margen/base) * 100
            except ZeroDivisionError:
                return 0.00

            for section in self.section_ids:
                if section.amount_from <= utility_percentage <= section.amount_to:
